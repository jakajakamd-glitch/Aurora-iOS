...
struct Compiler
{
    ...
    uint32_t compileFunction(AstExprFunction* func, uint8_t& protoflags)
    {
        ...
        // DISABLED: roblox's luaD_precall checks proto->flags != 0 and calls
        // proto+0x28 (native entry) which is NULL for our protos → crash/error.
    ...
    int32_t getConstantIndex(AstExpr* node)
    {
        ...
        case Constant::Type_Vector:
            cid = bytecode.addConstantVectorf(c->valueVector[0], c->valueVector[1], c->valueVector[2], c->valueVector[3]);
            break;
    ...
    void compileExprConstant(AstExpr* node, const Constant* cv, uint8_t target)
    {
        ...
        case Constant::Type_Vector:
        {
            int32_t cid = bytecode.addConstantVectorf(cv->valueVector[0], cv->valueVector[1], cv->valueVector[2], cv->valueVector[3]);
            if (cid < 0)
                CompileError::raise(node->location, "Exceeded constant limit; simplify the code to compile");
