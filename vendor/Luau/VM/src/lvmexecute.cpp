...
#define VM_INTERRUPT() \
    { \
        /* do NOT call g->cb.interrupt — wrong offset */ \
    }
...
LUAU_NOINLINE void luau_callhook(lua_State* L, lua_Hook hook, void* userdata)
{
    ...
    lua_Debug ar;
    ar.currentline =
        cl->tt == 7 ? -1 : luaG_getline((FFlag::LuauCIProto ? L->ci->p : cl->l.p), pcRel(L->ci->savedpc, (FFlag::LuauCIProto ? L->ci->p : cl->l.p)));
...
reentry:
    ...
    dispatch:
        ...
        if (SingleStep)
        {
            // do NOT call g->cb.debugstep — wrong offset
        }
...
#endif
        {
            ...
            VM_CASE(LOP_GETTABLEKS)
            {
                ...
                else
                {
                    ...
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = fasttm(L, uvalue(rb)->metatable, TM_INDEX)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                    ...
                    else if (ttisvector(rb))
                    {
                        ...
                        if (fn && ttisfunction(fn) && clvalue(fn)->tt == 7)
                        {
                            // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                            LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                            StkId top = L->top;
            ...
            VM_CASE(LOP_SETTABLEKS)
            {
                ...
                else
                {
                    ...
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = fasttm(L, uvalue(rb)->metatable, TM_NEWINDEX)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                ...
                if (ttype(ra) == ttype(rb))
                {
                    ...
                    case LUA_TUSERDATA:
                        ...
                        if (uvalue(ra)->metatable == uvalue(rb)->metatable)
                        {
                            ...
                            else if (ttisfunction(fn) && clvalue(fn)->tt == 7)
                            {
                                // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                                LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                                StkId top = L->top;
            ...
            VM_CASE(LOP_ADD)
            {
                ...
                else
                {
                    ...
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_ADD)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_SUB)
            {
                ...
                else
                {
                    // fast-path for userdata with C functions
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_SUB)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_MUL)
            {
                ...
                else
                {
                    ...
                    const TValue* fn = 0;
                    if (ttisuserdata(rbc) && (fn = luaT_gettmbyobj(L, rbc, TM_MUL)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_DIV)
            {
                ...
                else
                {
                    ...
                    StkId rbc = ttisnumber(rb) ? rc : rb;
                    const TValue* fn = 0;
                    if (ttisuserdata(rbc) && (fn = luaT_gettmbyobj(L, rbc, TM_DIV)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_IDIV)
            {
                ...
                else
                {
                    ...
                    StkId rbc = ttisnumber(rb) ? rc : rb;
                    const TValue* fn = 0;
                    if (ttisuserdata(rbc) && (fn = luaT_gettmbyobj(L, rbc, TM_IDIV)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_MULK)
            {
                ...
                else
                {
                    // fast-path for userdata with C functions
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_MUL)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_DIVK)
            {
                ...
                else
                {
                    // fast-path for userdata with C functions
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_DIV)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_IDIVK)
            {
                ...
                else
                {
                    // fast-path for userdata with C functions
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_IDIV)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
                        LUAU_ASSERT(L->top + 3 < L->stack + L->stacksize);
                        StkId top = L->top;
            ...
            VM_CASE(LOP_MINUS)
            {
                ...
                else
                {
                    // fast-path for userdata with C functions
                    const TValue* fn = 0;
                    if (ttisuserdata(rb) && (fn = luaT_gettmbyobj(L, rb, TM_UNM)) && ttisfunction(fn) && clvalue(fn)->tt == 7)
                    {
                        // note: it's safe to push arguments past top for complicated reasons (see top of the file)
            ...
            VM_CASE(LOP_BREAK)
            {
                ...
                // do NOT call g->cb.debugbreak — wrong offset
            ...
            VM_CASE(LOP_CMPPROTO)
            {
                ...
                Closure* ccl = clvalue(ra);
                if (ccl->tt == 7 || ccl->l.p->funid != funid)
                    pc += LUAU_INSN_D(insn) - 1;
...
int luau_precall(lua_State* L, StkId func, int nresults)
{
    ...
    if (ccl->tt != 7)
    {
        ...
        // roblox's Proto has codeentry at +0x18 (separate from code at +0x30).
        // codeentry is the actual execution entry point. Use it for savedpc.
        const Instruction* codeentry = *(const Instruction**)((char*)p + 0x18);
        ci->savedpc = codeentry ? codeentry : p->code;
