...
// GC disabled — our static GC can't run on roblox's global_State because
// the GC internal state (gcstate, gray lists, page lists) is managed by
// roblox's own GC.  running our luaC_step would corrupt that state.
// roblox's lua_newthread already calls luaC_checkGC internally, so we
// just skip it here.
