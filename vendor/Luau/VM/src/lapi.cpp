...
lua_State* lua_newthread(lua_State* L)
{
    ...
    ensure_stack(L, 1);
    // luaE_newthread allocates ExtraSpace via luaM_new_ (GC-managed)
    // and copies shared/identity/capabilities from parent.
...
int lua_gc(lua_State* L, int what, int data)
{
    ...
    {
        // GC values are expressed in Kbytes: #bytes/2^10
        res = int(g->gcstepsize) >> 10;
        g->gcstepsize = double(data << 10);
        break;
    }
    case LUA_GCISPAUSED:
    {
