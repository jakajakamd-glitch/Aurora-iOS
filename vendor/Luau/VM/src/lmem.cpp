...
// static_assert removed — rbx LuaTable is 48 bytes, ours is 56 with gclist, "size mismatch for table header");
...
{
    // roblox uses mimalloc — bypass our page allocator (freepages/freegcopages
    // are at wrong offsets in our global_State struct, accessing them corrupts
    // roblox memory). Call g->frealloc directly.
    global_State* g = L->global;

    void* block = (*g->frealloc)(g->ud, NULL, 0, nsize);
    if (block == NULL && nsize > 0)
        luaD_throw(L, LUA_ERRMEM);

    // ZERO-FILL — mimalloc doesn't guarantee zeroed memory
    memset(block, 0, nsize);

    // do NOT update g->totalbytes or g->memcatbytes — our struct has totalbytes
    // at G+0x50 (correct now) but memcatbytes is at a wrong offset. Writing to
    // it would corrupt roblox memory. Roblox's frealloc tracks memory internally.
    (void)memcat;
...
{
    // roblox uses mimalloc, not luau's page allocator.  bypass the page
    // allocator and call g->frealloc directly.
    global_State* g = L->global;
    ...
    // ZERO-FILL the block — mimalloc does NOT guarantee zeroed memory.
    // without this, LuaTable.gclist and LuaTable.metatable are garbage,
    // causing roblox's VM to crash when it reads them.
    memset( block, 0, nsize );

    // do NOT update g->totalbytes or g->memcatbytes — memcatbytes is at a
    // wrong offset in our struct, writing to it corrupts roblox memory.
    (void)memcat;
...
void luaM_free_(lua_State* L, void* block, size_t osize, uint8_t memcat)
{
    ...
    // do NOT update g->totalbytes or g->memcatbytes — memcatbytes at wrong offset
    (void)memcat;
}
...
void* luaM_realloc_(lua_State* L, void* block, size_t osize, size_t nsize, uint8_t memcat)
{
    ...
    // roblox uses mimalloc, not luau's page allocator.  our page-based
    // newblock/freeblock would read incompatible lua_Page metadata from
    // roblox-allocated blocks and crash.  bypass the page allocator
    // entirely and call g->frealloc directly for all allocations.
    void* result = (*g->frealloc)(g->ud, block, osize, nsize);
    ...
    // do NOT update g->totalbytes or g->memcatbytes — memcatbytes at wrong offset
    // do NOT call g->cb.onallocate — our lua_Callbacks struct is at the
    // wrong offset, so cb.onallocate reads garbage and jumps to a bad addr.
    (void)memcat;
