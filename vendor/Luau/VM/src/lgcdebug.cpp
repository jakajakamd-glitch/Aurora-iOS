...
{
    fprintf(
        f, "{\"type\":\"function\",\"cat\":%d,\"size\":%d", cl->memcat, cl->tt == 7 ? int(sizeCclosure(cl->nupvalues)) : int(sizeLclosure(cl->nupvalues))
    );
...
static void dumpthread(FILE* f, lua_State* th)
{
    ...
    if (FFlag::LuauCIProto ? (cip != nullptr && cip->source) : (tcl && tcl->tt != 7 && tcl->l.p->source))
    {
...
static void enumthread(EnumContext* ctx, lua_State* th)
{
    ...
    if (FFlag::LuauCIProto ? (cip && cip->source) : (tcl && tcl->tt != 7 && tcl->l.p->source))
    {
        Proto* p = (FFlag::LuauCIProto ? cip : tcl->l.p);
