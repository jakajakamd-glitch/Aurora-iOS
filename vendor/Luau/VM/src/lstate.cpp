...
lua_State* luaE_newthread(lua_State* L)
{
    ...
    L1->userdata = (RobloxExtraSpace*)luaM_new_(L, sizeof(RobloxExtraSpace), L->activememcat);
    if (L1->userdata)
    {
        memset(L1->userdata, 0, sizeof(RobloxExtraSpace));

        void* parent_es = L->userdata;
        LOGD( "newthread: L=%p is_main=%d parent_es=%p", L, L == L->global->mainthread, parent_es );

        if (parent_es)
        {
            // ES+0x18 = std::shared_ptr<Shared> = {ptr(0x18), ctrl(0x20)}
            // Copy BOTH 8 bytes of the shared_ptr
            void* shared_ptr = *(void**)((char*)parent_es + 0x18);
            void* shared_ctrl = *(void**)((char*)parent_es + 0x20);
            LOGD( "newthread: shared_ptr=%p shared_ctrl=%p", shared_ptr, shared_ctrl );

            *(void**)((char*)L1->userdata + 0x18) = shared_ptr;
            *(void**)((char*)L1->userdata + 0x20) = shared_ctrl;

            if (shared_ptr)
            {
                // getIdentity: identity = Shared[0x10]
                void* identity = *(void**)((char*)shared_ptr + 0x10);
                LOGD( "newthread: identity(Shared+0x10)=%p", identity );
                if (identity)
                    LOGD( "newthread: identity[0x68]=%p", *(void**)((char*)identity + 0x68) );
                else
                    LOGE( "newthread: identity is NULL!" );
            }
            else
                LOGE( "newthread: shared_ptr is NULL!" );
        }
        else
            LOGE( "newthread: parent_es is NULL!" );
    }
...
{
    global_State* g = L->global;
    // do NOT call g->cb.userthread — wrong offset
    (void)g;
