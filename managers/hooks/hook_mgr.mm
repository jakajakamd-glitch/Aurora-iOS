...
static void* find_trampoline_slot(uintptr_t page_lo, uintptr_t page_hi,
                                  uintptr_t avoid_lo, uintptr_t avoid_hi,
                                  size_t slots_needed) {
    uint32_t* lo_ptr = (uint32_t*)page_lo;
    uint32_t* hi_ptr = (uint32_t*)page_hi;
    size_t run = 0;
    uint32_t* run_start = nullptr;

    for (uint32_t* p = lo_ptr; p < hi_ptr; ++p) {
        ...
        if (*p == 0xD503201Fu || *p == 0x00000000u) {
...
void hook_mgr_type::hook(uintptr_t absolute_address, void *replacement, void **backup) {
    ...
    if (kr != KERN_SUCCESS) {
        NSLog(OBF_NS("[Aurora] hook_mgr: COW failed kr=%d target=%p"), kr, (void*)target);
        return;
    }
    ...
    size_t tramp_written = relocate_arm64(relocated, original_insns, kOverwrittenSlots,
                                          target, tramp_addr);
    ...
    int64_t b_back_off = (int64_t)(target + kPatchBytes) - (int64_t)(tramp_addr + kOverwrittenSlots * 4);
    ...
    *(uint32_t*)((uintptr_t)tramp_slot + kPatchBytes) = b_back;
    ...
    sys_icache_invalidate((void*)target, kPatchBytes);
    sys_icache_invalidate(tramp_slot, kTrampSlots * 4);
    ...
    if (kr != KERN_SUCCESS) {
        NSLog(OBF_NS("[Aurora] hook_mgr: RX restore failed kr=%d"), kr);
        return;
    }
